...
Let $\{X_t, Y_t\}_{t=1}^T$ denote a multivariate time series process where $X_t \in \mathbb{R}^d$ represents exogenous covariates and $Y_t \in \mathbb{R}$ is the target variable. Given historical observation trajectory $\mathcal{H}_T = \{(X_t, Y_t)\}_{t=1}^T$, our goal is to forecast the future sequence over prediction horizon $H$:
