...
def calculate_special_bonus(employee: pd.DataFrame) -> pd.DataFrame:
    return employee.assign(
        bonus = lambda x: x['salary'].where(
            x['name'].str.startswith('M') & (x['employee_id'] % 2 == 1), 0
        )
    )
```
...
def patients_without_condition(patients: pd.DataFrame) -> pd.DataFrame:
    return patients[~patients['conditions'].str.contains('DIAB100', na = False)]
```
