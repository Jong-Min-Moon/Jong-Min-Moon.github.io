...
\begin{document}
	...
	% Slide 6: Dual-Layer Agent - Outer Layer
		 ...
		 \vskip 5mm
			...
			\resizebox{\textwidth}{!}{%
				\begin{tikzpicture}[
					...
					\node (llm) [sidebox] at (4.8, 0) {\textbf{LLM} develops new heuristics, according to a \textbf{fixed prompt set} $\mathcal{P} = P_1, \ldots, $};
	...
	Repeat   for a fixed number of time; the whole procedure can be written as $\mathcal{H}= \texttt{inner}(\texttt{prompt})$		
			% End of resizebox
		\end{frame}
	...
	\begin{frame}{Differential Memory for Directional Evolution}
		Unlike traditional random mutations, LLM-LNS uses a directed approach to learn from past generations[cite: 2246].
		
		\begin{itemize}
			\item The LLM is provided with strategy-thought-fitness tuples from parent generations[cite: 2249, 2252].
			\item A meta-prompt instructs the LLM to learn the differences between higher- and lower-performing strategies[cite: 2257].
			\item This allows the LLM to act as an optimizer, identifying the most promising directions for code modification and avoiding past mistakes[cite: 2057, 2248].
		\end{itemize}
	\end{frame}
