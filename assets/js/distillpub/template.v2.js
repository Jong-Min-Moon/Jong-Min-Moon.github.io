...
function getLogger() {
  if (!loggerDiv) {
    ...
    loggerDiv.id = 'distill-logger-div';
    loggerDiv.style.cssText = 'position:fixed;top:0;left:0;z-index:99999;background:white;color:black;border:2px solid red;max-height:50vh;overflow:auto;width:100%';
    document.body.prepend(loggerDiv);
  }
  return loggerDiv;
}
console.log = function () { oldLog.apply(console, arguments); var msg = Array.from(arguments).join(' '); getLogger().innerHTML += '<p>LOG: ' + msg + '</p>'; };
console.error = function () { oldError.apply(console, arguments); var msg = Array.from(arguments).join(' '); getLogger().innerHTML += '<p>ERR: ' + msg + '</p>'; };
console.warn = function () { oldWarn.apply(console, arguments); var msg = Array.from(arguments).join(' '); getLogger().innerHTML += '<p>WARN: ' + msg + '</p>'; };
(function (factory) {
...
`);
  ...
  function optionalComponents(dom, data) {
    ...
    if (!title) {
      ...
      byline.parentNode.insertBefore(title, byline);
    }
